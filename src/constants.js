export const SIGN_IN = "SIGN_IN";
export const SIGN_OUT = "SIGN_OUT";
export const GAUTH_SIGN_IN = "GAUTH_SIGN_IN";
export const GAUTH_SIGN_OUT = "GAUTH_SIGN_OUT";

export const LOCAL = "LOCAL";
export const GOOGLE = "GOOGLE";
export const STUDENT = "student";
export const TEACHER = "teacher";
export const FETCH_PROFILE = "FETCH_PROFILE";

export const SIGNUP_LOAD = "SIGNUP_LOAD";
export const LOGIN_LOAD = "LOGIN_LOAD";

export const CREATE_CLASS = "CREATE_CLASS";
export const FETCH_CLASS = "FETCH_CLASS";
export const FETCH_CLASSES = "FETCH_CLASSES";
export const DELETE_CLASS = "FETCH_CLASS";
export const JOIN_CLASS = "JOIN_CLASS";
export const LEAVE_CLASS = "LEAVE_CLASS";

export const FETCH_TEST = "FETCH_TEST";
export const FETCH_TESTS = "FETCH_TESTS";
export const CREATE_TEST = "CREATE_TEST";
export const DELETE_TEST = "DELETE_TEST";
export const SUBMIT_TEST = "SUBMIT_TEST";
export const EDIT_TEST = "EDIT_TEST";

export const FETCH_ASSIGNMENT = "FETCH_ASSIGNMENT";
export const FETCH_ASSIGNMENTS = "FETCH_ASSIGNMENTS";
export const DELETE_ASSIGNMENT = "DELETE_ASSIGNMENTS";
export const SUBMIT_ASSIGNMENT = "SUBMIT_ASSIGNMENT";
export const CREATE_ASSIGNMENT = "CREATE_ASSIGNMENT";
export const EDIT_ASSIGNMENT = "EDIT_ASSIGNMENT";

export const SNACKBAR_SUCCESS = "SNACKBAR_SUCCESS";
export const SNACKBAR_ERROR = "SNACKBAR_ERROR";
export const SNACKBAR_INFO = "SNACKBAR_WARNING";
export const SNACKBAR_CLOSE = "SNACKBAR_CLOSE";
